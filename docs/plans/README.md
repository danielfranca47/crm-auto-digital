...
| `motor-llm-otimizacoes.md` | 4 achados da Fase 3 de `prompt-engineering-principles`: temperature não configurada, Structured Outputs (json_schema+strict) da OpenAI, isolamento da mensagem do lead contra prompt injection, reordenação do prompt para prompt caching automático |
