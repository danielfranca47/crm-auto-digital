...
| `auto_items` | Lista ordenada de itens automáticos da **1ª decisão** do turno: `{type:"text", content, source:"sales_flow", source_label:"Fluxo de Venda"}` ou `{type:"media", media_url, media_type}` — só blocos `send_message`/`send_media` do Fluxo de Venda (`_send_actions`). Nunca contém conteúdo de uma 2ª decisão reenfileirada (ver `requeue_items` abaixo) | `_send_actions` despachados pelo runner via `_send_sales_flow_action()` |
| `requeue_items` | Lista ordenada (mesmo shape de `auto_items`) da **2ª decisão** de uma saudação composta (`requeue_pending_message` — ver "Saudação composta" em [`llm-architecture.md`](llm-architecture.md)), já pré-ordenada internamente pela mesma regra de `phase_trigger_fired` (mídia/mensagem antes do texto quando o `phase_trigger` da 2ª decisão dispara; texto antes, senão). Item de texto usa `source:"child_llm"` (`source_label` = nome da fase, ex. "Apresentação") ou `source:"fallback"` (`source_label="Handoff (erro de decisão)"` quando a 2ª decisão caiu em `llm_failure`). Frontend revela `requeue_items` sempre **depois** do turno completo da 1ª decisão — nunca misturado com `auto_items`/`phase_trigger_fired` da 1ª, para não inverter a ordem visual (bug corrigido em `fix-playground-ordem-saudacao-composta`, 19/08/2026) | Novo job `whatsapp.inbound.n8n` criado por `_dispatch_system_actions` (`routes/executor.py`), processado pelo worker no ciclo seguinte — ordem cronológica correta por construção (job só é criado depois da 1ª resposta já ter sido enviada) |
| `phase_trigger_fired` | `bool` — só reflete a **1ª decisão**; quando `True`, frontend exibe `auto_items` **antes** da resposta LLM da 1ª decisão. Nunca contaminado pelo `phase_trigger` de uma 2ª decisão reenfileirada | Runner envia `_send_actions` antes da mensagem LLM quando `phase_trigger` disparou |
