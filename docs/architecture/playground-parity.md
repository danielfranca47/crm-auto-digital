...
| `auto_items` | Lista ordenada de itens automáticos: `{type:"text", content}` ou `{type:"media", media_url, media_type}` — do Fluxo de Venda (`_send_actions`) **ou** de uma pergunta comercial pendente extraída pela Filha Recepção (`requeue_pending_message` — ver "Saudação composta" em [`llm-architecture.md`](llm-architecture.md)) | `_send_actions` despachados pelo runner via `_send_sales_flow_action()`; para `requeue_pending_message`, um novo job real processado pelo worker no ciclo seguinte |
