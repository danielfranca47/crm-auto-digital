...
- Tratamento de mensagens de mídia (imagem/áudio) no monitoramento — hoje
  `handle_monitor_inbound` ignora mensagens sem texto; isso também limita o
  que a classificação de estágio (acima) consegue enxergar e o que a tela de
  leitura (acima) consegue exibir. Ver
