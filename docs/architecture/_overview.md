...
| [`whatsapp-connection.md`](whatsapp-connection.md) | Conexão da instância WhatsApp: QR code e código de pareamento, `ConnectResponse`, sanitização de telefone, normalização de status, `ConexaoNumero.tsx`; credenciais UazAPI (`UAZAPI_ADMIN_TOKEN` vs. `instance_token`) e rotação | Novo método de vínculo, novo campo em `ConnectResponse`, mudança no timeout/polling do frontend, mudança em onde `UAZAPI_ADMIN_TOKEN` é usado |
...
| [`leads-schema.md`](leads-schema.md) | Schema de nome do lead: regra "companyName OU contactName obrigatório" (CHECK + Pydantic + form), pontos de criação e seus fallbacks, convenção de exibição `leadDisplayName`, `wa_display_name` (nome de perfil do WhatsApp); `origin` (direção inbound/outbound, único campo lido pela IA) x `acquisition_channel` (canal de marketing, texto livre, não lido pela IA); endpoint de upload de planilha (auth, isolamento por usuário, limite de tamanho, limpeza periódica de arquivos antigos) | Novo ponto de criação de lead, mudança na regra de nome obrigatório, novo componente de exibição do nome do lead, mudança na classificação de direção ou no Select `LEAD_DIRECTION_OPTIONS`, mudança no endpoint de upload/import (auth, storage, limites, limpeza) |
| [`dynamic-variables.md`](dynamic-variables.md) | Variáveis dinâmicas `{{chave}}`: catálogo de sistema, `custom_variables`, motor de resolução (`variable_resolver.py`), pontos de entrada em `enrich_context_bundle`, UI de atalho `/` (`VariableTextarea`/`VariablePicker`) | Nova variável de sistema, novo campo de texto que deve suportar `{{}}`, mudança no motor de resolução ou no picker |
...
| [`whatsapp-send-resiliencia.md`](whatsapp-send-resiliencia.md) | Resiliência do envio WhatsApp: retry exponencial em 429/503 (`uazapi_client.py`), validação E.164 do número antes do envio (`whatsapp_send.py`) | Mudança na política de retry, novo status code retryable, mudança na validação de número |
| [`auth-email.md`](auth-email.md) | Auth e gestão de utilizadores: endpoints de register/login/forgot-password/reset/change-password, login sem senha via OTP (agent-local) com lockout anti-força-bruta e rate-limit anti-spam de envio, modelo User (campos), PasswordResetToken, email SMTP via Resend, rotas públicas frontend-crm, painel admin de utilizadores | Novo endpoint de auth, mudança no modelo User, mudança no fluxo/limite do OTP, novo campo de config SMTP, novo template de email, nova rota pública |
...
| Auth endpoints, User model, password reset, email service, rotas públicas do frontend-crm, login sem senha (OTP), lockout anti-força-bruta e rate-limit anti-spam de envio | `auth-email.md` |
...
| Regra de nome obrigatório do lead (companyName/contactName), novo ponto de criação de lead, convenção de exibição `leadDisplayName`, `wa_display_name` | `leads-schema.md` |
| Classificação de direção do lead (`origin`, inbound/outbound), campo `acquisition_channel`, `LEAD_DIRECTION_OPTIONS`/`formatLeadOriginLabel`, `ProspectConfirmModal` | `leads-schema.md` (+ `pipeline-phases.md` se afetar o prompt) |
| Endpoint `POST /api/uploads` (auth, isolamento por usuário, limite de tamanho), resolução de `upload_id` em `/api/assistente-ia/processar`/`/preview`, limpeza periódica de uploads antigos (`upload_cleanup.py`) | `leads-schema.md` |
| Nova variável de sistema `{{}}`, campo de texto novo que deve suportar variáveis, mudança no picker `/` ou em `variable_resolver.py` | `dynamic-variables.md` |
