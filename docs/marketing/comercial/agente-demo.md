...
**Ajuste de expectativa no roteiro de teste:** o Cenário 4 (pergunta de preço) ainda depende inteiramente do que está escrito em `custom_instructions`/`offer_description`, por causa do `appointment_mode` — revisar essa resposta no Playground com atenção redobrada. O Cenário 3 (recuperação de paciente sumido) deixou de depender só de `custom_instructions` — o disparo automático por inatividade já existe de verdade (`followup_auto_trigger_enabled`/`followup_auto_trigger_inactivity_days` no AI Profile; ver `docs/architecture/followup.md`). Atenção: esse gatilho corre no reconciliador do `backend-crm` (varredura periódica fora do Playground) — o roteiro continua simulando só a resposta textual da IA após o silêncio; para validar o disparo automático em si é preciso ligar o toggle e aguardar o ciclo do reconciliador, não só simular a mensagem no Playground.
