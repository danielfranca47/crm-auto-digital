...
- `POST /whatsapp-instances/connect`: endpoint interno protegido por `X-Service-Token` para iniciar conexão e obter QR/paircode via Uazapi.
- `GET /whatsapp-instances/status?instance_id=`: endpoint interno protegido por `X-Service-Token` para consultar status na Uazapi.
...
Campos extras no body são repassados para a Uazapi (ex.: `systemName`, `fingerprintProfile`).
...
Para **webhook por instância**, o Core usa `token: <INSTANCE_TOKEN>` quando `instance_token` é fornecido; caso contrário, usa `admintoken`.
