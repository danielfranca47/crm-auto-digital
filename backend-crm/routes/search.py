...
def executar_pesquisa(req: SearchRequest) -> Dict[str, Any]:
    """
    Dispara a automação e retorna o manifest + link do validado.
    """
    try:
        manifest = run_site_search(req.dict())
...
def baixar_validado(run_id: str):
    """
...
def baixar_arquivo(run_id: str, kind: Literal["csv", "xlsx", "xlsx_validado"]):
    man = _load_manifest(run_id)
...
@router.get("/runs")
def listar_runs() -> List[Dict[str, Any]]:
    """
