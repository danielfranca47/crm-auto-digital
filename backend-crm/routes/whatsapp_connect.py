...
def _set_whatsapp_webhook(*, user_id: int, instance_id: str) -> None:
    webhook_url = _build_webhook_url()
    masked = _mask_webhook_url(webhook_url)
    logger.info(
        "whatsapp webhook set user_id=%s instance_id=%s url=%s",
        user_id,
        instance_id,
        masked,
    )
...
def connect_whatsapp(current_user: CurrentUser = Depends(require_crm_access)):
    ...
    logger.info("whatsapp connect start user_id=%s instance_id=%s", current_user.id, instance_id)
    try:
        raw = connect_core_whatsapp_instance(current_user.id, instance_id)
    except HTTPException as exc:
        if exc.status_code and exc.status_code >= 500:
            logger.warning(
                "whatsapp connect failed user_id=%s instance_id=%s detail=%s",
                current_user.id,
                instance_id,
                exc.detail,
            )
            ...
            logger.info(
                "whatsapp reinit user_id=%s old_instance_id=%s new_instance_id=%s",
                current_user.id,
                instance_id,
                new_instance_id,
            )
    ...
    qr = _extract_qr(raw)
    logger.info("whatsapp connect success user_id=%s instance_id=%s", current_user.id, instance_id)
    _set_whatsapp_webhook(user_id=current_user.id, instance_id=instance_id)
...
def whatsapp_status(current_user: CurrentUser = Depends(require_crm_access)):
    ...
    logger.info("whatsapp status start user_id=%s instance_id=%s", current_user.id, instance_id)
    try:
        raw = status_core_whatsapp_instance(instance_id)
    except HTTPException as exc:
        if exc.status_code and exc.status_code >= 500:
            logger.warning(
                "whatsapp status failed user_id=%s instance_id=%s detail=%s",
                current_user.id,
                instance_id,
                exc.detail,
            )
    ...
    logger.info("whatsapp status success user_id=%s instance_id=%s", current_user.id, instance_id)
