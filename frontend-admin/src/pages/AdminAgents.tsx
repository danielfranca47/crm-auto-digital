...
import type { AgentModeOverview, AgentStage, UserProfile, UserDetail } from "@/services/api";
...
import {
  RefreshCw, Clock, User, Check, ChevronsUpDown,
  MessageSquare, ListChecks, Repeat2, Target,
} from "lucide-react";
...
function StageSheet({ state, onClose }: { state: SheetState | null; onClose: () => void }) {
  ...
  return (
    <Sheet open={!!state} onOpenChange={(o) => !o && onClose()}>
      <SheetContent side="right" className="w-[420px] sm:w-[520px] bg-slate-900 border-slate-700 text-slate-100">
        <SheetHeader className="mb-4">
          <div className="flex items-center gap-2 mb-1">
            <Icon size={16} className="text-slate-400" />
            <span className="text-xs text-slate-500 uppercase tracking-wide">{modeName}</span>
          </div>
          <SheetTitle className="text-slate-200 text-lg">{stage.label}</SheetTitle>
          ...
          <div className="space-y-5">
            {stage.tone_rule && (
              <div className="rounded-lg bg-slate-800/60 p-4">
                <p className="text-xs text-slate-500 uppercase tracking-wide mb-2">Regra de tom</p>
                <p className="text-sm text-slate-300 leading-relaxed">{stage.tone_rule}</p>
              </div>
            )}
                ...
                <ol className="space-y-2">
                  {stage.sample_questions.map((q, i) => (
                    <li key={i} className="flex gap-2">
                      <span className="text-xs text-slate-600 w-5 shrink-0 pt-0.5">{i + 1}.</span>
            ...
            {stage.followup_outcomes && Object.keys(stage.followup_outcomes).length > 0 && (
              <div className="rounded-lg bg-slate-800/60 p-4">
                <p className="text-xs text-slate-500 uppercase tracking-wide mb-3">Outcomes de follow-up</p>
                <div className="space-y-2">
                  {Object.entries(stage.followup_outcomes).map(([outcome, description]) => (
                    <div key={outcome} className="flex gap-2">
                      <Badge variant="secondary" className="bg-slate-700 text-slate-400 text-xs shrink-0 h-fit mt-0.5">
                        {outcome}
                      </Badge>
                ...
                <div className="space-y-3">
                  {Object.entries(diff).map(([field, { default: def, user }]) => (
                    <div key={field} className="text-sm">
                      <p className="text-slate-400 font-mono text-xs mb-1">{field}</p>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="rounded bg-slate-800/60 px-2 py-1">
                          <p className="text-[10px] text-slate-600 mb-0.5">Padrão</p>
                          <p className="text-xs text-slate-400">
                            {def === null || def === undefined ? "—" : Array.isArray(def) ? (def as unknown[]).join(", ") || "—" : String(def)}
                          </p>
                        </div>
                        <div className="rounded bg-amber-900/30 px-2 py-1">
                          <p className="text-[10px] text-amber-600 mb-0.5">Usuário</p>
                          <p className="text-xs text-amber-300">
                            {user === null || user === undefined ? "—" : Array.isArray(user) ? (user as unknown[]).join(", ") || "—" : String(user)}
                          </p>
                        </div>
                      </div>
                    </div>
...
function StageColumn({
  stage, color, onClick, isCustomized,
}: {
  stage: AgentStage; color: string; onClick: () => void; isCustomized?: boolean;
}) {
  const Icon = STAGE_ICON[stage.key] ?? MessageSquare;
  const hasData =
    stage.tone_rule ||
    ...
    (stage.followup_attempts && stage.followup_attempts.length > 0) ||
    (stage.followup_outcomes && Object.keys(stage.followup_outcomes).length > 0);
  const borderClass = MODE_BORDER_CLASSES[color] ?? "border-slate-700";
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed mb-3">{stage.description}</p>
      {stage.sample_questions && stage.sample_questions.length > 0 && (
        <p className="text-[11px] text-slate-600 line-clamp-1">{stage.sample_questions.length} perguntas</p>
      )}
      {stage.followup_attempts && stage.followup_attempts.length > 0 && (
        <p className="text-[11px] text-slate-600">{stage.followup_attempts.length} tentativas</p>
      )}
      {!hasData && <p className="text-[11px] text-slate-700">Sem dados específicos</p>}
      <p className="text-[10px] text-slate-600 group-hover:text-slate-400 mt-3 transition-colors">Ver detalhes →</p>
    </button>
  );
}
        ...
        <div className="flex items-center gap-3 flex-1 text-left">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge variant="secondary" className={`text-[10px] h-4 px-1.5 ${badgeClass}`}>
            ...
            <p className="text-xs text-slate-500 line-clamp-1">{mode.description}</p>
          </div>
          <div className="flex items-center gap-3 text-xs text-slate-600 mr-2">
            <span>{stages.length} estágios</span>
            {mode.max_chars && <span>{(mode.max_chars / 1000).toFixed(0)}k chars</span>}
          </div>
        </div>
      </AccordionTrigger>
          ...
          {stages.map((stage) => {
            ...
            return (
              <StageColumn
                ...
                onClick={() => onStageClick({ stage, modeName: mode.name, diff })}
              />
            );
          })}
        </div>
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <div className="flex items-center gap-2 truncate">
            ...
            {selected ? (
              <span className="truncate">{selected.email || `Usuário #${selected.user_id}`}</span>
            ) : (
      ...
      <PopoverContent className="w-[320px] p-0 bg-slate-800 border-slate-700" align="start">
        <Command className="bg-transparent">
          <CommandInput placeholder="Buscar usuário..." className="text-slate-300 placeholder:text-slate-500 border-slate-700" />
          <CommandList>
            <CommandEmpty className="text-slate-500 text-sm py-4 text-center">Nenhum usuário encontrado.</CommandEmpty>
            <CommandGroup>
              {selectedId && (
                <CommandItem onSelect={() => { onSelect(null); setOpen(false); }} className="text-slate-400 hover:text-slate-200 cursor-pointer">
              ...
              {users.map((u) => (
                ...
                >
                  <Check size={13} className={`mr-2 shrink-0 ${u.user_id === selectedId ? "opacity-100 text-indigo-400" : "opacity-0"}`} />
                  <div className="flex-1 truncate">
                    <p className="text-sm truncate">{u.email || `#${u.user_id}`}</p>
                    <p className="text-xs text-slate-500 truncate">
                      {u.agent_mode ?? "—"} · {u.template_key ?? "—"}
                      {u.has_custom_config && <span className="text-amber-500 ml-1">· personalizado</span>}
                    </p>
                  </div>
                </CommandItem>
              ))}
...
export default function AdminAgents() {
  ...
  const sortedModes = selectedUserId && userDetail
    ? [
        ...agentModes.filter(
          (m) => m.key === userDetail.playbook.key || m.canonical_agent_mode === userDetail.playbook.canonical_agent_mode
        ),
        ...agentModes.filter(
          (m) => m.key !== userDetail.playbook.key && m.canonical_agent_mode !== userDetail.playbook.canonical_agent_mode
        ),
  ...
  const queriedAt = overview?.queried_at ? formatTs(overview.queried_at) : null;

  return (
    <div className="p-6 max-w-5xl space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-slate-200">Agentes e Prompts</h1>
          <p className="text-slate-500 text-sm">Configuração ao vivo dos agentes — lida do estado real do sistema.</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        <UserCombobox users={users} selectedId={selectedUserId} onSelect={setSelectedUserId} />
        ...
        {selectedUserId && userDetail && (
          <div className="flex items-center gap-2">
            {userDetail.has_custom_config ? (
              <Badge className="bg-amber-900/40 text-amber-300 border-amber-700/40 text-xs">configuração personalizada</Badge>
            ) : (
        ...
        <Accordion type="multiple" className="space-y-0">
          {sortedModes.map((mode) => (
            <AgentModeCard
              ...
              userDetail={
                selectedUserId && userDetail &&
                (userDetail.playbook.key === mode.key || userDetail.playbook.canonical_agent_mode === mode.canonical_agent_mode)
                  ? userDetail
                  : null
              }
      ...
      {!overviewLoading && !overviewError && agentModes.length === 0 && (
        <div className="rounded-lg bg-slate-800/40 border border-slate-700 px-6 py-10 text-center">
          <p className="text-slate-500 text-sm">Nenhum agente encontrado. Verifique se os playbooks estão carregados.</p>
        </div>
      )}
        ...
        <Card className="bg-slate-800/40 border-slate-700">
          <CardContent className="px-5 py-4">
            <div className="flex items-center justify-between">
              <p className="text-xs text-slate-500 uppercase tracking-wide">Usuários com AI profile configurado</p>
              <span className="text-sm font-semibold text-slate-300">{users.length}</span>
            </div>
            <div className="flex flex-wrap gap-1.5 mt-3">
              {users.map((u) => (
                ...
                >
                  {u.email ?? `#${u.user_id}`}
                  {u.has_custom_config && <span className="text-amber-400">·</span>}
                </button>
              ))}
            </div>
          </CardContent>
