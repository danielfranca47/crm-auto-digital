...
export default function AdminInstances() {
  ...
  const handleReconnect = async (inst: AdminInstance) => {
    setReconnecting((prev) => new Set(prev).add(inst.instance_id));
    try {
      const res = await api.reconnectInstance(inst.instance_id);
      if (res.ok) {
        toast({
      ...
      toast({
        title: "Erro",
        description: err instanceof Error ? err.message : "Erro ao reconectar.",
        variant: "destructive",
      });
    } finally {
