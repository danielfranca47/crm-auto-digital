...
### Validação manual via curl.exe (Windows)

Após criar um job, você pode inspecionar o estado com:

```powershell
curl.exe -H "X-Service-Token: $env:CRM_SERVICE_TOKEN" `
  http://localhost:8000/api/jobs/<JOB_ID>
```

Campos esperados:
- `status`: `pending` (requeue) ou `failed` (após max attempts)
- `attempts`: incrementa a cada claim
- `scheduled_at`: futuro em requeue (backoff 60s/180s)
- `started_at`: deve ficar `null` após requeue/fail
- `completed_at`: preenchido quando `status=failed`
- `error`: contém detalhes (`phase`, `service`, `http_status`, `retryable`)
