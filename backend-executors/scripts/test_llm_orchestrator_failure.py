...
def main() -> None:
    ...
    def child_with_nulls(_route: str, _prompt: str) -> str:
        return (
            '{"message_text":"ok","did_complete_phase":false,'
            '"recommended_next_category":"null","outcome":"null",'
            '"kanban_highlight":"null","signals":[],"confidence":0.5}'
    ...
    decision_engine.llm_service.generate_mother_route = ok_mother
    decision_engine.llm_service.generate_child_result = child_with_nulls
    handler.records.clear()
    decision = decision_engine.decide(context, logger=logger)
    messages = [record.getMessage() for record in handler.records]
    if any("event=llm_orchestrator_error" in msg for msg in messages):
        raise AssertionError(f"unexpected llm_orchestrator_error: {messages}")
