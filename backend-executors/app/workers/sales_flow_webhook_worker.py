...
from app.runners.sales_flow_webhook import execute_job
...
def _parse_types() -> list[str]:
    raw = os.getenv("SALES_FLOW_WEBHOOK_WORKER_TYPES", "sales_flow.webhook.dispatch")
...
def main() -> int:
    ...
    types = _parse_types()
    max_jobs = _parse_optional_int("SALES_FLOW_WEBHOOK_WORKER_MAX_JOBS")
    max_runtime_seconds = _parse_optional_int("SALES_FLOW_WEBHOOK_WORKER_MAX_RUNTIME_SECONDS")
    ...
    while True:
        ...
        try:
            ...
            logger.error("event=job_poll_error error=%s", exc, extra={"phase": "poll"})
        ...
        if not job:
            sleep_seconds = POLL_BACKOFF_SECONDS[min(backoff_index, len(POLL_BACKOFF_SECONDS) - 1)]
            logger.info("event=no_job_available sleep=%ss", sleep_seconds, extra={"phase": "poll"})
            backoff_index = min(backoff_index + 1, len(POLL_BACKOFF_SECONDS) - 1)
            time.sleep(sleep_seconds)
            continue
