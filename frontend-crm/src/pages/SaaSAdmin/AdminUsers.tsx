...
export default function AdminUsers() {
  ...
  const loadUsers = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await api.admin.listUsers(search || undefined);
      setUsers(data);
    } catch (err: any) {
      toast({
        title: "Erro ao listar usuários",
        description: err?.message ?? "Verifique o token admin.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const saveExtensions = async () => {
    ...
    setIsSaving(true);
    try {
      await api.admin.setExtensions(modalUser.id, editExtensions);
      setUsers((prev) =>
        prev.map((u) =>
